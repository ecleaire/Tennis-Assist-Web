...
function playTextGlitch(target) { if (!target || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return; window.clearTimeout(dataGlitchTimer); document.querySelectorAll(".is-glitch-revealing").forEach((element) => element.classList.remove("is-glitch-revealing")); void target.offsetWidth; target.classList.add("is-glitch-revealing"); dataGlitchTimer = window.setTimeout(() => target.classList.remove("is-glitch-revealing"), 1100); }
function showPublicPane(name, animate = false) { const loggedInHome = name === "login-panel" && Boolean(session); document.querySelectorAll(".public-pane").forEach((pane) => pane.classList.toggle("hidden", loggedInHome || pane.id !== name)); $("viewer").classList.toggle("hidden", !loggedInHome); document.querySelectorAll(".public-nav button").forEach((button) => button.classList.toggle("active", button.id === `public-${name.replace("public-", "")}-tab`)); if (animate) playTextGlitch(loggedInHome ? $("viewer") : $(name)); }
...
$("public-login-tab").addEventListener("click", () => showPublicPane("login-panel", true)); $("public-links-tab").addEventListener("click", () => showPublicPane("public-links", true)); $("public-rules-tab").addEventListener("click", () => showPublicPane("public-rules", true)); document.querySelectorAll("[data-rule-pdf-src]").forEach((link) => link.addEventListener("click", (event) => { const source = link.dataset.rulePdfSrc; if (!source) return; event.preventDefault(); $("rule-pdf-frame").src = source; $("rule-pdf-note").textContent = "選択した公式ルールPDFを表示しています。表示されない場合はボタンを長押しして別タブで開いてください。"; })); $("rule-pdf-fullscreen").addEventListener("click", async () => { try { await $("rule-pdf-viewer").requestFullscreen(); } catch { window.open($("rule-pdf-frame").src, "_blank", "noopener"); } }); $("rule-search-button").addEventListener("click", () => { const query = $("rule-search").value.trim(); if (query) $("rule-pdf-frame").src = `https://drive.google.com/file/d/1JMmggxMfSWABUcA5sbM9U3qffb-ZMb2U/preview#search=${encodeURIComponent(query)}`; }); renderPublicLinks();
