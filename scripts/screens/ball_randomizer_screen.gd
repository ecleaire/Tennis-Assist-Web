...
func _update_playfield_size(schedule_transform: bool = true) -> void:
	...
	if playfield_portrait:
		...
		var max_width: float = minf(820.0, maxf(320.0, available_size.x - horizontal_padding)) if dashboard_limited else minf(1440.0 if is_fullscreen_ui else 1680.0, height_limited_width)
