...
func _apply_responsive_controls(available_size: Vector2) -> void:
	...
	var button_size: Vector2 = Vector2(108, 38) if portrait else (Vector2(132, 44) if dashboard_limited or is_fullscreen_ui else Vector2(180, 52))
	...
	var field_margin_size: int = 6 if portrait else (10 if dashboard_limited or is_fullscreen_ui else 18)
...
func _update_playfield_size(schedule_transform: bool = true) -> void:
	...
	var dashboard_limited: bool = _uses_dashboard_layout_limits()
	var horizontal_padding: float = 20.0 if dashboard_limited else (28.0 if is_fullscreen_ui else 40.0)
	...
	if playfield_portrait:
		...
		var horizontal_padding_portrait: float = 12.0 if dashboard_limited else (16.0 if is_fullscreen_ui else 18.0)
		var reserved_height: float = 88.0 if dashboard_limited else (190.0 if is_fullscreen_ui else 165.0)
		...
		var reserved_height_landscape: float = 150.0 if is_fullscreen_ui else 118.0
		var height_limited_width: float = maxf(360.0, (available_size.y - reserved_height_landscape) * PLAYFIELD_RATIO)
		var fullscreen_width_limit: float = 1280.0
		var max_content_width: float = maxf(320.0, available_size.x - horizontal_padding)
		var max_width: float = minf(820.0, max_content_width) if dashboard_limited else minf(minf(fullscreen_width_limit if is_fullscreen_ui else 1680.0, height_limited_width), max_content_width)
