...
const animationSettingsStorageKey = "tennis-assist-history-animation-v1";
const defaultAnimationSettings = { enabled: true, intervalMinutes: 3, durationSeconds: 0.7 };
function clamp(value, min, max) { return Math.min(max, Math.max(min, Number(value) || min)); }
function loadAnimationSettings() { try { const saved = JSON.parse(localStorage.getItem(animationSettingsStorageKey) || "null"); return saved ? { enabled: saved.enabled !== false, intervalMinutes: clamp(saved.intervalMinutes ?? defaultAnimationSettings.intervalMinutes, 0.5, 60), durationSeconds: clamp(saved.durationSeconds ?? defaultAnimationSettings.durationSeconds, 0.2, 3) } : { ...defaultAnimationSettings }; } catch { return { ...defaultAnimationSettings }; } }
let animationSettings = loadAnimationSettings();
let dataGlitchTimer = 0;
let periodicGlitchTimer = 0;
function dataModeActive() { return Boolean(session) && !$("viewer").classList.contains("hidden"); }
function playTextGlitch(target, force = false) { if (!target || (!force && !animationSettings.enabled) || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return; window.clearTimeout(dataGlitchTimer); document.documentElement.style.setProperty("--data-glitch-duration", `${animationSettings.durationSeconds}s`); document.querySelectorAll(".is-glitch-revealing").forEach((element) => element.classList.remove("is-glitch-revealing")); void target.offsetWidth; target.classList.add("is-glitch-revealing"); dataGlitchTimer = window.setTimeout(() => target.classList.remove("is-glitch-revealing"), animationSettings.durationSeconds * 1000 + 350); }
function schedulePeriodicGlitch() { window.clearInterval(periodicGlitchTimer); periodicGlitchTimer = 0; if (!animationSettings.enabled || !dataModeActive()) return; periodicGlitchTimer = window.setInterval(() => { if (dataModeActive() && document.visibilityState === "visible") playTextGlitch($("viewer")); }, animationSettings.intervalMinutes * 60000); }
function showPublicPane(name, animate = false) { const loggedInHome = name === "login-panel" && Boolean(session); document.querySelectorAll(".public-pane").forEach((pane) => pane.classList.toggle("hidden", loggedInHome || pane.id !== name)); $("viewer").classList.toggle("hidden", !loggedInHome); document.querySelectorAll(".public-nav button").forEach((button) => button.classList.toggle("active", button.id === `public-${name.replace("public-", "")}-tab`)); if (animate && loggedInHome) playTextGlitch($("viewer")); schedulePeriodicGlitch(); }
...
function dateBounds() { const value = $("period").value; const now = new Date(); const today = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime(); let from = 0; let to = Number.POSITIVE_INFINITY; if (value === "today") { from = today; to = today + 86400000 - 1; } else if (value === "yesterday") { from = today - 86400000; to = today - 1; } else if (value === "day-before-yesterday") { from = today - 2 * 86400000; to = today - 86400000 - 1; } else if (value === "week" || value === "month") { from = Date.now() - (value === "week" ? 7 : 31) * 86400000; } if ($("date-from").value) from = Math.max(from, new Date(`${$("date-from").value}T00:00:00`).getTime()); if ($("date-to").value) to = Math.min(to, new Date(`${$("date-to").value}T23:59:59.999`).getTime()); return { from, to }; }
...
function render() {
  ...
  const optionsMarkup = `<option value="">すべてのチーム</option>${allTeams.map((team) => `<option value="${escapeHtml(team)}">${escapeHtml(team)}</option>`).join("")}`;
  if ($("team-filter").dataset.options !== optionsKey) { $("team-filter").innerHTML = optionsMarkup; $("team-filter").value = selected; $("team-filter").dataset.options = optionsKey; }
  if ($("stats-team").dataset.options !== optionsKey) { $("stats-team").innerHTML = `<option value="">チームを選択</option>${allTeams.map((team) => `<option value="${escapeHtml(team)}">${escapeHtml(team)}</option>`).join("")}`; $("stats-team").value = statsTeam; $("stats-team").dataset.options = optionsKey; }
...
function syncAnimationSettingsForm() { $("glitch-enabled").checked = animationSettings.enabled; $("glitch-interval-minutes").value = animationSettings.intervalMinutes; $("glitch-duration-seconds").value = animationSettings.durationSeconds; $("glitch-interval-minutes").disabled = !animationSettings.enabled; $("glitch-duration-seconds").disabled = !animationSettings.enabled; $("animation-settings-status").textContent = ""; }
function openAnimationSettings() { syncAnimationSettingsForm(); $("animation-settings-dialog").showModal(); }
function saveAnimationSettings(event) { event.preventDefault(); animationSettings = { enabled: $("glitch-enabled").checked, intervalMinutes: clamp($("glitch-interval-minutes").value, 0.5, 60), durationSeconds: clamp($("glitch-duration-seconds").value, 0.2, 3) }; localStorage.setItem(animationSettingsStorageKey, JSON.stringify(animationSettings)); document.documentElement.style.setProperty("--data-glitch-duration", `${animationSettings.durationSeconds}s`); schedulePeriodicGlitch(); $("animation-settings-status").textContent = animationSettings.enabled ? `${animationSettings.intervalMinutes}分間隔で保存しました。` : "グリッチ演出を無効にしました。"; window.setTimeout(() => $("animation-settings-dialog").close(), 450); }
async function login(event) { event.preventDefault(); const password = normalizedPassword($("password").value); const account = managedAccounts[password]; if (!account) { $("login-status").textContent = "管理者パスワードを確認してください。"; return; } let apiKey = text($("api-key").value); if (!apiKey) { try { const saved = JSON.parse(localStorage.getItem("tennis-assist-admin-v1") || "{}"); apiKey = text(saved.apiKey); } catch { apiKey = ""; } } if (!apiKey) apiKey = password === "shukugawa" ? "GAS" : password; if ($("remember-login").checked) localStorage.setItem(loginStorageKey, JSON.stringify({ remember: true, password, apiKey })); else localStorage.removeItem(loginStorageKey); session = { account, apiKey, columns: [] }; $("login-status").textContent = "履歴を読み込んでいます..."; try { await loadHistory(); $("account-label").textContent = account.label; $("account-label").classList.remove("hidden"); $("logout").classList.remove("hidden"); $("login-status").textContent = ""; showPublicPane("login-panel"); playTextGlitch($("viewer")); } catch (error) { session = null; $("login-status").textContent = `${error.message}。APIキーを入力して再試行してください。`; } }
...
$("login-form").addEventListener("submit", login); $("login-form").addEventListener("keydown", (event) => { if (event.key !== "Enter" || event.isComposing) return; event.preventDefault(); $("login-form").requestSubmit(); }); $("logout").addEventListener("click", logout); $("refresh").addEventListener("click", async () => { try { $("history-status").textContent = "再読み込み中..."; await loadHistory(); playTextGlitch($("viewer")); } catch (error) { $("history-status").textContent = error.message; } }); ["period", "date-from", "date-to", "stats-team", "team-filter", "result-filter", "kind-filter", "sort-filter"].forEach((id) => $(id).addEventListener("change", () => { render(); if (id === "kind-filter") playTextGlitch($("viewer")); }));
$("public-login-tab").addEventListener("click", () => showPublicPane("login-panel", true)); $("public-links-tab").addEventListener("click", () => showPublicPane("public-links")); $("public-rules-tab").addEventListener("click", () => showPublicPane("public-rules")); $("animation-settings-open").addEventListener("click", openAnimationSettings); $("animation-settings-close").addEventListener("click", () => $("animation-settings-dialog").close()); $("animation-settings-form").addEventListener("submit", saveAnimationSettings); $("glitch-enabled").addEventListener("change", () => { const enabled = $("glitch-enabled").checked; $("glitch-interval-minutes").disabled = !enabled; $("glitch-duration-seconds").disabled = !enabled; }); $("glitch-preview").addEventListener("click", () => { const previewSettings = animationSettings; animationSettings = { enabled: true, intervalMinutes: clamp($("glitch-interval-minutes").value, 0.5, 60), durationSeconds: clamp($("glitch-duration-seconds").value, 0.2, 3) }; playTextGlitch($("animation-settings-dialog"), true); animationSettings = previewSettings; }); document.querySelectorAll("[data-rule-pdf-src]").forEach((link) => link.addEventListener("click", (event) => { const source = link.dataset.rulePdfSrc; if (!source) return; event.preventDefault(); $("rule-pdf-frame").src = source; $("rule-pdf-note").textContent = "選択した公式ルールPDFを表示しています。表示されない場合はボタンを長押しして別タブで開いてください。"; })); $("rule-pdf-fullscreen").addEventListener("click", async () => { try { await $("rule-pdf-viewer").requestFullscreen(); } catch { window.open($("rule-pdf-frame").src, "_blank", "noopener"); } }); $("rule-search-button").addEventListener("click", () => { const query = $("rule-search").value.trim(); if (query) $("rule-pdf-frame").src = `https://drive.google.com/file/d/1JMmggxMfSWABUcA5sbM9U3qffb-ZMb2U/preview#search=${encodeURIComponent(query)}`; }); document.documentElement.style.setProperty("--data-glitch-duration", `${animationSettings.durationSeconds}s`); renderPublicLinks();
try { const savedLogin = JSON.parse(localStorage.getItem(loginStorageKey) || "null"); if (savedLogin && savedLogin.remember === true) { $("password").value = text(savedLogin.password); $("api-key").value = text(savedLogin.apiKey); $("remember-login").checked = true; window.setTimeout(() => $("login-form").requestSubmit(), 0); } } catch { localStorage.removeItem(loginStorageKey); }
