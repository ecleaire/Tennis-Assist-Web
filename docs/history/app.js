...
let entryGlitchTimer = 0;
function finishEntryGlitch() { const overlay = $("entry-glitch"); if (!overlay || !overlay.classList.contains("is-active")) return; overlay.classList.add("is-finished"); window.setTimeout(() => overlay.classList.remove("is-active", "is-finished"), 180); }
function playEntryGlitch() { const overlay = $("entry-glitch"); if (!overlay || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return; window.clearTimeout(entryGlitchTimer); overlay.classList.remove("is-active", "is-finished"); void overlay.offsetWidth; overlay.classList.add("is-active"); entryGlitchTimer = window.setTimeout(finishEntryGlitch, 1400); }
...
$("entry-glitch")?.addEventListener("animationend", (event) => { if (event.animationName === "entry-glitch-wipe") finishEntryGlitch(); });
