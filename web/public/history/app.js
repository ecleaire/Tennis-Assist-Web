...
function playDataGlitch() { const viewer = $("viewer"); if (!viewer || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return; window.clearTimeout(dataGlitchTimer); viewer.classList.remove("is-glitch-revealing"); void viewer.offsetWidth; viewer.classList.add("is-glitch-revealing"); dataGlitchTimer = window.setTimeout(() => viewer.classList.remove("is-glitch-revealing"), 1100); }
